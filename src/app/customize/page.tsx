...
import { Upload, Download, Trash2, ShoppingCart } from "lucide-react"
...
// Design area bounds (within t-shirt printable area)
const PRINT_AREA = {
  left: 130,
  top: 80,
  width: 140,
...
// T-shirt paths extracted from SVG mockups
// Front SVG viewBox: 0 0 708.03 632.28
// Back SVG viewBox: 0 0 708.23 632.88
// Paths are split into outline (stroke) and fill areas for dynamic coloring
...
// Front side: fill paths (cls-1 in SVG) - these get the dynamic color
...
// Back side: main outline path (no class in SVG)
...
// Back side: fill paths (cls-1 in SVG) - these get the dynamic color
...
// Scale factor to fit 708x632 SVG into 400x500 canvas
// We use 0.55 to leave room for the full t-shirt shape on canvas
const MOCKUP_SCALE = 0.55
// Offset to center the scaled mockup on the 400x500 canvas
const MOCKUP_OFFSET_X = 200
const MOCKUP_OFFSET_Y = 300
...
export default function CustomizePage() {
  ...
  const [designs, setDesigns] = useState<{ front: DesignElement[]; back: DesignElement[] }>({
    front: [],
    back: [],
  })
  ...
  const createTshirtBg = useCallback((canvas: Canvas, color: string, side: DesignSide) => {
    ...
    console.log(`[tshirt] Creating ${side} mockup with color ${color}, ${fillPaths.length} fill paths`)

    // Single path: fill entire t-shirt body + stroke for line details
    // The outline path contains outer silhouette + internal sub-paths
    // With fill, the entire shape is filled. With stroke, line details appear.
    ...
    console.log(`[tshirt] Total canvas objects: ${canvas.getObjects().length}`)
    canvas.renderAll()
  }, [])

  // Initialize Fabric canvas
  useEffect(() => {
    if (!canvasRef.current || fabricCanvasRef.current || isInitializing.current) return
    ...
    createTshirtBg(canvas, tshirtColor, "front")

    // Create clip path for print area
  ...
  const handleImageUpload = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (event) => {
        ...
        // Create HTML Image element for Fabric.js v7
        const imgElement = new Image()
        imgElement.onload = () => {
          const fabricImg = new FabricImage(imgElement, {
          ...
          // Auto-scale to fit within print area
          const maxSize = 120
  ...
  const handleExportPreview = useCallback(() => {
    ...
    link.download = `tshirt-${activeSide}-preview.png`
    link.href = dataURL
    link.click()
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            <Tabs value={activeSide} onValueChange={(v) => setActiveSide(v as DesignSide)}>
              ...
              <div className="relative border-2 border-dashed rounded-lg overflow-hidden bg-muted/30 flex items-center justify-center p-4">
          ...
          <Card>
            <CardContent className="pt-6 space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold">{formatPrice(UNIT_PRICE)}</span>
              </div>
              <div className="flex justify-between items-center text-lg font-bold">
                <span>Total</span>
                <span>{formatPrice(UNIT_PRICE * quantity)}</span>
              </div>
        ...
        <DialogContent>
          ...
          <div className="flex flex-col gap-4">
            <div className="text-sm text-muted-foreground">
              ...
              <p>
                Color: {TSHIRT_COLORS.find((c) => c.value === tshirtColor)?.name}
              </p>
            </div>
          </div>
